/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   wall.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mfeldman <mfeldman@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/04 15:36:06 by fllanet           #+#    #+#             */
/*   Updated: 2024/03/13 22:42:03 by mfeldman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/cub3D.h"

static	bool	test_around(char **map, int y, int x, char *charset) //big return
{
	if (map[y - 1][x] != '1' && !is_char_in_set(map[y - 1][x], charset))
		return (1);
	if (map[y][x - 1] != '1' && !is_char_in_set(map[y][x - 1], charset))
		return (1);
	if (map[y][x + 1] != '1' && !is_char_in_set(map[y][x + 1], charset))
		return (1);
	if (map[y + 1][x] != '1' && !is_char_in_set(map[y + 1][x], charset))
		return (1);
	return (0);
}

static	bool	is_void_around(t_map *map, char *charset)
{
	int	y;
	int	x;

	y = 1;
	while (y < map->height - 1)
	{
		x = 1;
		while (x < map->width - 1)
		{
			if (is_char_in_set(map->map[y][x], charset))
			{
				if (test_around(map->map, y, x, charset))
					return (1);
			}
			x++;
		}
		y++;
	}
	return (0);
}

bool	is_closed_by_wall(t_map *map) // one main fct 
{
	int		i;

	i = 0;
	// 2 fct 
	while (i < map->width)
	{
		if ((map->map[0][i] != '1' && map->map[0][i] != ' ')
			|| (map->map[map->height - 1][i] != '1'
			&& map->map[map->height - 1][i] != ' '))
			return (false);
		i++;
	}
	i = 0;
	while (i < map->height)
	{
		if ((map->map[i][0] != '1' && map->map[i][0] != ' ')
			|| (map->map[i][map->width - 1] != '1'
			&& map->map[i][map->width - 1] != ' '))
			return (false);
		i++;
	}
	if (is_void_around(map, "0NSEW"))
		return (false);
	return (true);
}
